...
class IntegerValid extends AnyFunSuite {
  ...
  test("zero") {
    val Right(result) = arktekk.oppgave3.toml.parseAll("""d1 = 0
      |d2 = +0
      |d3 = -0
      |
      |h1 = 0x0
  ...
  test("long") {
    val Right(result) =
      arktekk.oppgave3.toml.parseAll("""# int64 "should" be supported, but is not mandatory. It's fine to skip this
      |# test.
  ...
  test("literals") {
    val Right(result) = arktekk.oppgave3.toml.parseAll("""bin1 = 0b11010110
      |bin2 = 0b1_0_1
      |
      |oct1 = 0o01234567
      |oct2 = 0o755
  ...
  test("integer") {
    val Right(result) = arktekk.oppgave3.toml.parseAll("""answer = 42
      |posanswer = +42
      |neganswer = -42
      |zero = 0
      |""".stripMargin): @unchecked
  ...
  test("underscore") {
    val Right(result) = arktekk.oppgave3.toml.parseAll("""kilo = 1_000
      |x = 1_1_1_1
      |""".stripMargin): @unchecked
    val Right(expectedJson) = parseJson("""{
